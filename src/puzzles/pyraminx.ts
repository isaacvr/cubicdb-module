  ...
  const cycles = {
    U: (count: number) => {
      ...
      for (let i = 0; i < times; i += 1) {
        const NF = [0, -1, -1, 3, -1, 5, 6, -1, -1].map((v, p) =>
          v < 0 ? faces.F[p] : faces.R[v],
        );
        const NR = [1, -1, -1, 4, -1, 3, 7, -1, -1].map((v, p) =>
          v < 0 ? faces.R[p] : faces.L[v],
        );
        const NL = [-1, 0, -1, 5, 3, -1, -1, 6, -1].map((v, p) =>
          v < 0 ? faces.L[p] : faces.F[v],
        );
        const ND = [...faces.D];
        update(NF, NR, NL, ND);
      }
    ...
    R: (count: number) => {
      const times = ((count % 3) + 3) % 3;
      for (let i = 0; i < times; i += 1) {
        const NF = [-1, -1, 0, -1, 5, 3, -1, -1, 6].map((v, p) =>
          v < 0 ? faces.F[p] : faces.D[v],
        );
        const NR = [-1, 2, -1, 4, 5, -1, -1, 8, -1].map((v, p) =>
          v < 0 ? faces.R[p] : faces.F[v],
        );
        const NL = [...faces.L];
        const ND = [1, -1, -1, 4, -1, 3, 7, -1, -1].map((v, p) =>
          v < 0 ? faces.D[p] : faces.R[v],
        );
        update(NF, NR, NL, ND);
      }
    },
    L: (count: number) => {
      const times = ((count % 3) + 3) % 3;
      for (let i = 0; i < times; i += 1) {
        const NF = [-1, 0, -1, 5, 3, -1, -1, 6, -1].map((v, p) =>
          v < 0 ? faces.F[p] : faces.L[v],
        );
        const NR = [...faces.R];
        const NL = [1, -1, -1, 4, -1, 3, 7, -1, -1].map((v, p) =>
          v < 0 ? faces.L[p] : faces.D[v],
        );
        const ND = [-1, 1, -1, 3, 4, -1, -1, 7, -1].map((v, p) =>
          v < 0 ? faces.D[p] : faces.F[v],
        );
        update(NF, NR, NL, ND);
      }
    },
    B: (count: number) => {
      ...
      for (let i = 0; i < times; i += 1) {
        const NF = [...faces.F];
        const NR = [-1, -1, 2, -1, 4, 5, -1, -1, 8].map((v, p) =>
          v < 0 ? faces.R[p] : faces.D[v],
        );
        const NL = [-1, -1, 2, -1, 4, 5, -1, -1, 8].map((v, p) =>
          v < 0 ? faces.L[p] : faces.R[v],
        );
        const ND = [-1, -1, 2, -1, 4, 5, -1, -1, 8].map((v, p) =>
          v < 0 ? faces.D[p] : faces.L[v],
        );
        update(NF, NR, NL, ND);
      }
    },
