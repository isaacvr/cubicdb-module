...
exports.randomCube = randomCube;
exports.solve = solve;
exports.initFull = initFull;
...
function setComb(arr, idxC, mask, isEdge) {
    ...
    var r = 4, fill = end;
    for (var i = end; i >= 0; i--) {
        if (idxC >= mathlib_1.Cnk[i][r]) {
            idxC -= mathlib_1.Cnk[i][r--];
...
var CubieCube = /** @class */ (function () {
    ...
    CubieCube.EdgeMult = function (a, b, prod) {
        for (var ed = 0; ed < 12; ed += 1) {
            prod.ea[ed] = a.ea[b.ea[ed] >> 1] ^ (b.ea[ed] & 1);
        }
    };
    CubieCube.CornMult = function (a, b, prod) {
        for (var corn = 0; corn < 8; corn += 1) {
    ...
    CubieCube.prototype.isEqual = function (c) {
        if (this.ca.some(function (v, i) { return v != c.ca[i]; }) || this.ea.some(function (v, i) { return v != c.ea[i]; })) {
            return false;
        }
        return true;
    };
...
var CoordCube = /** @class */ (function () {
    ...
    CoordCube.prototype.calcPruning = function () {
        this.prun = Math.max(Math.max(getPruningMax(CoordCube.UDSliceTwistPrunMax, UDSliceTwistPrun, this.twist * N_SLICE + UDSliceConj[this.slice][this.tsym]), getPruningMax(UDSliceFlipPrunMax, UDSliceFlipPrun, this.flip * N_SLICE + UDSliceConj[this.slice][this.fsym])), Math.max(USE_CONJ_PRUN
            ? getPruningMax(TwistFlipPrunMax, TwistFlipPrun, ((this.twistc >> 3) << 11) | FlipS2RF[this.flipc ^ (this.twistc & 7)])
            : 0, USE_TWIST_FLIP_PRUN
    ...
    CoordCube.prototype.doMovePrun = function (cc, m) {
        ...
        this.twist >>= 3;
        this.prun = Math.max(Math.max(getPruningMax(CoordCube.UDSliceTwistPrunMax, UDSliceTwistPrun, this.twist * N_SLICE + UDSliceConj[this.slice][this.tsym]), getPruningMax(UDSliceFlipPrunMax, UDSliceFlipPrun, this.flip * N_SLICE + UDSliceConj[this.slice][this.fsym])), USE_TWIST_FLIP_PRUN
            ? getPruningMax(TwistFlipPrunMax, TwistFlipPrun, (this.twist << 11) | FlipS2RF[(this.flip << 3) | (this.fsym ^ this.tsym)])
            : 0);
        return this.prun;
    };
...
var Search = /** @class */ (function () {
    ...
    Search.prototype.initPhase2 = function (phase2Cubie) {
        ...
        var prun = Math.max(getPruningMax(EPermCCombPPrunMax, EPermCCombPPrun, p2edge * N_COMB + CCombPConj[Perm2CombP[p2corn] & 0xff][SymMultInv[p2esym][p2csym]]), getPruningMax(MCPermPrunMax, MCPermPrun, p2corn * N_MPERM + MPermConj[p2mid][p2csym]));
...
function initBasic() {
    ...
    }
    {
        ...
        setFunc, getFunc, multFunc, ud2std) {
            for (var i = 0; i < N_SIZE; i++) {
                ...
                for (var j = 0; j < N_MOVES; j++) {
                    multFunc(c, moveCube[ud2std ? ud2std[j] : j], d_1);
                    moveTable[i][j] = d_1[getFunc]();
                }
            }
        }
        ...
        CubieCube.CornMult, ud2std);
        for (var i = 0; i < N_SLICE; i++) {
            UDSliceMove[i] = [];
            UDSliceConj[i] = [];
            c.setUDSlice(i);
            for (var j = 0; j < N_MOVES; j++) {
                CubieCube.EdgeMult(c, moveCube[j], d_1);
                UDSliceMove[i][j] = d_1.getUDSlice();
            }
            for (var j = 0; j < 16; j += 2) {
                CubieCube.EdgeConjugate(c, SymMultInv[0][j], d_1);
                UDSliceConj[i][j >> 1] = d_1.getUDSlice();
            }
        }
        MPermMove = [];
        for (var i = 0; i < N_MPERM; i++) {
            ...
            for (var j = 0; j < N_MOVES2; j++) {
                CubieCube.EdgeMult(c, moveCube[ud2std[j]], d_1);
                MPermMove[i][j] = d_1.getMPerm();
            }
            for (var j = 0; j < 16; j++) {
                CubieCube.EdgeConjugate(c, SymMultInv[0][j], d_1);
                MPermConj[i][j] = d_1.getMPerm();
            }
        }
        for (var i = 0; i < N_COMB; i++) {
            ...
            c.setCComb(i % 70);
            for (var j = 0; j < N_MOVES2; j++) {
                CubieCube.CornMult(c, moveCube[ud2std[j]], d_1);
                CCombPMove[i][j] = d_1.getCComb() + 70 * (((P2_PARITY_MOVE >> j) & 1) ^ ~~(i / 70));
            }
            for (var j = 0; j < 16; j++) {
                CubieCube.CornConjugate(c, SymMultInv[0][j], d_1);
                CCombPConj[i][j] = d_1.getCComb() + 70 * ~~(i / 70);
            }
        }
    }
}
...
function randomCube() {
    ...
    do {
        ep = ~~(Math.random() * mathlib_1.fact[12]);
        cp = ~~(Math.random() * mathlib_1.fact[8]);
