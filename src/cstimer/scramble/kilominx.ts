...
function initPhase3() {
  ...
  createPrun(
    ...
    210 * 81 * 24,
    14,
    comb4FullMove.bind(null, Phase3Move),
    3,
    4,
    6,
  );
...
function idaSearch(
  ...
  for (let axis = 0; axis < N_AXIS; axis++) {
    ...
    for (let pow = 0; pow < 4; pow++) {
      ...
      if (
        idaSearch(idx1, isSolved, getPrun, doMove, N_AXIS, maxl - 1, axis, sol)
      ) {
        sol.push([axis, pow]);
...
function solveKiloCubie(cc: KiloCubie) {
  ...
  const sol1 = solve(
    ...
    function (idx: number[]) {
      return Math.max(
        getPruning(Phase1Prun, idx[0]),
        getPruning(Phase1Prun, idx[1]),
      );
    },
    function (idx: number[], move: number) {
      const idx1 = [
  ...
  const sol2 = solve(
    ...
    function (idx: number[]) {
      return Math.max(
        getPruning(Phase2Prun, idx[0]),
        getPruning(Phase2Prun, idx[1]),
      );
    },
    function (idx: number[], move: number) {
      const idx1 = [
