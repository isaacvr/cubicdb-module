...
exports.pochscramble = pochscramble;
exports.utilscramble = utilscramble;
...
function adjScramble(faces, adj, len, suffixes) {
    ...
    for (var j = 0; j < len; j++) {
        do {
            face = (0, mathlib_1.rn)(faces.length);
        } while ((used >> face) & 1);
        ret.push(faces[face] + (0, mathlib_1.rndEl)(suffixes));
        used &= ~adj[face];
        used |= 1 << face;
    }
    return ret.join(" ");
...
function yj4x4(type, len) {
    ...
    for (j = 0; j < len; j++) {
        var done = 0;
        do {
            var first = (0, mathlib_1.rn)(turns.length);
            var second = (0, mathlib_1.rn)(turns[first].length);
            if (first != lastaxis || donemoves[second] == 0) {
                if (first == lastaxis) {
                    donemoves[second] = 1;
                    var rs = (0, mathlib_1.rn)(cubesuff.length);
                    if (first == 0 && second == 0) {
                        fpos = (fpos + 4 + rs) % 4;
                    }
                    if (first == 1 && second == 2) {
...
(0, scramble_1.regScrambler)("444yj", yj4x4);
...
(0, scramble_1.regScrambler)("bic", bicube);
// Clock functions.
function c(s) {
    var array = [
        s + "=0",
    ...
    return " " + (0, mathlib_1.rndEl)(array) + " ";
}
function c2() {
    return (0, mathlib_1.rndEl)(["U", "d"]) + (0, mathlib_1.rndEl)(["U", "d"]);
}
function c3() {
    return "     ";
}
...
function pochscramble(x, y) {
    ...
    for (i = 0; i < y; i++) {
        ret += "  ";
        for (j = 0; j < x; j++) {
            ret += (j % 2 == 0 ? "R" : "D") + (0, mathlib_1.rndEl)(["++", "--"]) + " ";
        }
        ret += "U" + (ret.endsWith("-- ") ? "'\\n" : "\\n");
    }
    return ret;
...
function carrotscramble(x, y) {
    ...
    for (i = 0; i < y; i++) {
        ret += " ";
        for (j = 0; j < x / 2; j++) {
            ret += (0, mathlib_1.rndEl)(["+", "-"]) + (0, mathlib_1.rndEl)(["+", "-"]) + " ";
        }
        ret += "U" + (0, mathlib_1.rndEl)(["'\\n", "\\n"]);
    }
    return ret;
}
function gigascramble(len) {
    ...
    for (i = 0; i < Math.ceil(len / 10); i++) {
        ...
        for (j = 0; j < 10; j++) {
            ret +=
                (j % 2 == 0 ? "Rr".charAt((0, mathlib_1.rn)(2)) : "Dd".charAt((0, mathlib_1.rn)(2))) +
                    (0, mathlib_1.rndEl)(["+ ", "++", "- ", "--"]) +
                    " ";
        }
        ret += "y" + (0, mathlib_1.rndEl)(minxsuff) + "\n";
    }
    return ret;
}
...
function sq1_getseq(num, type, len) {
    for (var n = 0; n < num; n++) {
        ...
        var cnt = 0;
        while (cnt < len) {
            var x = (0, mathlib_1.rn)(12) - 5;
            var y = type == 2 ? 0 : (0, mathlib_1.rn)(12) - 5;
...
function moyuRedi(length) {
    ...
    for (var i = 0; i < length; i++) {
        ret.push((0, scramble_1.mega)([["R"], ["L"]], ["", "'"], 3 + (0, mathlib_1.rn)(3)));
    }
    return ret.join(" x ");
}
    ...
    switch (type) {
        ...
        case "clkwca": // Clock (WCA Notation)
            ...
            for (var i_2 = 0; i_2 < 14; i_2++) {
                ret = ret.replace("?", (0, mathlib_1.rndEl)(clkapp));
            }
            return ret
                .replace("?", (0, mathlib_1.rndEl)(["", " UR"]))
                .replace("?", (0, mathlib_1.rndEl)(["", " DR"]))
                .replace("?", (0, mathlib_1.rndEl)(["", " DL"]))
                .replace("?", (0, mathlib_1.rndEl)(["", " UL"]));
        ...
        case "clkc": // Clock (concise)
            ...
            for (var i_3 = 0; i_3 < 4; i_3++)
                ret += "(" + ((0, mathlib_1.rn)(12) - 5) + ", " + ((0, mathlib_1.rn)(12) - 5) + ") / ";
            for (var i_4 = 0; i_4 < 6; i_4++)
                ret += "(" + ((0, mathlib_1.rn)(12) - 5) + ") / ";
            for (var i_5 = 0; i_5 < 4; i_5++)
                ret += (0, mathlib_1.rndEl)(["d", "U"]);
            return ret;
        ...
        case "pyrm": // Pyraminx (random moves)
            ret = (0, scramble_1.mega)([["U"], ["L"], ["R"], ["B"]], ["!", "'"], len);
        ...
        case "r3": // multiple 3x3x3 relay
            ...
            return (0, scramble_1.formatScramble)(ret);
        ...
        case "sq2": // Square-2
            var i = 0;
            while (i < len) {
                var rndu = (0, mathlib_1.rn)(12) - 5;
                var rndd = (0, mathlib_1.rn)(12) - 5;
                if (rndu != 0 || rndd != 0) {
                    i++;
                    ret += "(" + rndu + "," + rndd + ") / ";
                }
        ...
        case "-1": // -1x-1x-1 (micro style)
            for (var i_8 = 0; i_8 < len; i_8++) {
                ret += String.fromCharCode(32 + (0, mathlib_1.rn)(224));
            }
            ret += "Error: subscript out of range";
            return ret;
        case "333noob": // 3x3x3 for noobs
            ret = (0, scramble_1.mega)([
        ...
        case "lol": // LOL
            ret = (0, scramble_1.mega)([["L"], ["O"]], 0, len);
            return ret.replace(/ /g, "");
    }
}
(0, scramble_1.regScrambler)([
    "15p",
    "15pm",
    "15pat",
    "clkwca",
