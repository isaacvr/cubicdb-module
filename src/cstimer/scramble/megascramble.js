...
function megascramble(type, length) {
    ...
    switch (value.length) {
        case 1:
            return (0, scramble_1.mega)(value[0], [""], length);
        case 2:
            return (0, scramble_1.mega)(value[0], value[1], length);
        case 3:
            return (0, scramble_1.mega)(value[0], value[1], value[2]);
    }
}
...
function formatScramble(type, length) {
    ...
    return (0, scramble_1.formatScramble)(value);
}
for (var i in args) {
    (0, scramble_1.regScrambler)(i, megascramble);
}
for (var i in args2) {
    (0, scramble_1.regScrambler)(i, formatScramble);
}
for (var i in edges) {
    (0, scramble_1.regScrambler)(i, edgescramble);
}
function cubeNNN(type, len) {
    ...
    return (0, scramble_1.mega)(data, cubesuff, size * 10);
}
(0, scramble_1.regScrambler)("cubennn", cubeNNN);
function edge(start, end, moves, len) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        var trigger = (0, mathlib_1.rn)(8);
        var layer = (0, mathlib_1.rn)(2);
        var turn = (0, mathlib_1.rn)(3);
        scramble +=
            v +
                " " +
                triggers[trigger][0] +
    ...
    scramble += " " + (0, mathlib_1.rndEl)(end);
    return scramble;
}
// Megaminx
var epcord = new mathlib_1.coord("p", 6, -1);
var eocord = new mathlib_1.coord("o", 6, -2);
var cpcord = new mathlib_1.coord("p", 6, -1);
var cocord = new mathlib_1.coord("o", 6, -3);
function eMove(idx, m) {
    var perm = epcord.set([], idx >> 5);
    var twst = eocord.set([], idx & 0x1f);
    if (m == 0) {
        (0, mathlib_1.acycle)(twst, [0, 1, 2, 3, 4], 1);
        (0, mathlib_1.acycle)(perm, [0, 1, 2, 3, 4], 1);
    }
    else if (m == 1) {
        (0, mathlib_1.acycle)(twst, [0, 1, 2, 3, 5], 1);
        (0, mathlib_1.acycle)(perm, [0, 1, 2, 3, 5], 1);
    }
    else if (m == 2) {
        (0, mathlib_1.acycle)(twst, [1, 2, 3, 4, 5], 1, [0, 0, 0, 0, 1, 2]);
        (0, mathlib_1.acycle)(perm, [1, 2, 3, 4, 5]);
    }
...
function cMove(idx, m) {
    ...
    if (m == 0) {
        ...
        (0, mathlib_1.acycle)(twst, [0, 5, 1, 2, 3], 1, [2, 0, 0, 0, 0, 3]);
        (0, mathlib_1.acycle)(perm, [0, 5, 1, 2, 3]);
    }
    else if (m == 2) {
        (0, mathlib_1.acycle)(twst, [0, 2, 3, 4, 5], 1, [1, 0, 0, 0, 1, 3]);
        (0, mathlib_1.acycle)(perm, [0, 2, 3, 4, 5]);
    }
    return cpcord.get(perm) * 243 + cocord.get(twst);
}
var solv = new mathlib_1.Solver(3, 4, [
    [0, eMove, 32 * 360],
    [0, cMove, 243 * 360],
]);
function getMinxLSScramble(type, length, cases) {
    ...
    do {
        if (type == "mlsll") {
            edge = (0, mathlib_1.rn)(32 * 360);
            corn = (0, mathlib_1.rn)(243 * 360);
        }
        else if (type == "mgmpll") {
            edge = epcord.get((0, mathlib_1.rndPerm)(5, true).concat([5])) * 32;
            corn = cpcord.get((0, mathlib_1.rndPerm)(5, true).concat([5])) * 243;
        }
        else if (type == "mgmll") {
            var eo = eocord.set([], (0, mathlib_1.rn)(32));
            eo[0] += eo[5];
            eo[5] = 0;
            var co = cocord.set([], (0, mathlib_1.rn)(243));
            co[0] += co[5];
            co[5] = 0;
            edge = epcord.get((0, mathlib_1.rndPerm)(5, true).concat([5])) * 32 + eocord.get(eo);
            corn = cpcord.get((0, mathlib_1.rndPerm)(5, true).concat([5])) * 243 + cocord.get(co);
        }
    } while (edge == 0 && corn == 0);
...
(0, scramble_1.regScrambler)("mlsll", getMinxLSScramble)("mgmpll", getMinxLSScramble)("mgmll", getMinxLSScramble);
